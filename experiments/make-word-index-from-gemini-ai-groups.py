...
type PageCountDict = dict[str, int]
type TitleDict = dict[str, PageCountDict]
type WordDict = dict[str, TitleDict]
...
class SearchEngine:
    ...
    def add_page_content(self, title: str, content: dict[str, str]) -> None:
        for page, ai_text in content.items():
            words = normalize_string(ai_text)
            for word in words:
                if word not in self.index:
                    self.index[word] = {}
                if title not in self.index[word]:
                    self.index[word][title] = {}
                if page not in self.index[word][title]:
                    self.index[word][title][page] = 0
                self.index[word][title][page] += 1
...
def print_index(search_engine: SearchEngine, title: str) -> None:
    word_keys = sorted(search_engine.index.keys())
    for key in word_keys:
        print(key)

        title_info = search_engine.index[key]
        for title, page_count in title_info.items():
            print(f"  {title}")

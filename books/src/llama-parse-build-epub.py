...
def _add_cover_to_spine(book: epub.EpubBook, spine: list) -> None:
    ...
    ``ebooklib.EpubCoverHtml`` defaults to ``is_linear=False``, which makes the
    spine writer emit ``linear="no"`` regardless of any tuple override. EPUBCheck
    then raises OPF-096 because no content document hyperlinks to ``cover.xhtml``.
    Flipping ``is_linear`` keeps the cover in reading order and silences the rule.
    """

...
def roster_text(story_characters: Iterable[str] = (), story_things: Iterable[str] = ()) -> str:
    ...
    lines += [
        ...
        *[
        ...
        "  the wrong answer. Ask what the lettering DOES:",
        "    sound_effect  a noise, or words shouted by nobody the story names --",
        "                  an unseen crowd, two anonymous brawlers behind a fence,",
        "                  or a character's own yell painted beside them in frame.",
        "    dialogue      a named character's line that reaches somebody, most",
        "                  often arriving from off-panel with another character",
        "                  answering it in the same or the next panel.",
        "  Measured on Camera Crazy: the SAME words, `HELP! HELP!`, are both --",
        "  sound_effect on 155 where Donald is in frame yelling, dialogue two",
        "  panels later where it arrives off-panel and the nephews answer it.",
        "  Either way the speaker field still names who says it; the type says",

...
comic_prompt = """
Analyze the comic image, the provided OCR text locations, and the AI text extraction results
...
OCR Results and Text Boxes:
```json
{0}
```

Output Format (IMPORTANT - FOLLOW EXACTLY):
Return ONLY a JSON array of objects. No surrounding fields.
Each object MUST have these exact fields:

[
    {{
      "panel_id": "string",
      "text_bubble_id": "string",
      "box_ids": [string, ...],
      "split_cleaned_box_texts": {{
          "box_id_string": "cleaned_text_string",
          ...
      }},
...
- Do not return markdown. Do not use ```json fences.
- The root of the response MUST be the JSON array, not an object.
...
- But "cleaned_text" should have newlines so that the text matches the text format in the
  comic image.

Analyze the provided OCR data and image thoroughly to produce accurate and contextually appropriate
groupings with cleaned and corrected text that reflects the comic's essential narrative elements.
